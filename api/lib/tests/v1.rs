mod integration {
    ...
    async fn get_films_works() {
        ...
        let _ = repo.create_film(&film1).await;
        let _ = repo.create_film(&film2).await;
    ...
    async fn get_film_works() {
        ...
        let _ = repo.create_film(&film).await;
        ...
        assert_eq!(result.title, film.title);
        assert_eq!(result.director, film.director);
        assert_eq!(result.poster, film.poster);
        assert_eq!(result.year, film.year);
    ...
    async fn update_film_works() {
        ...
        let mut film = create_test_film("1");
        film.created_at = None;
        let created_file = repo.create_film(&film).await.unwrap();
